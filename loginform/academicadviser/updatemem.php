...
//update for inactive user
if (isset($_GET['inid'])) {
    $id = $_GET['inid'];
    $status = 0;
    $query = "UPDATE csgname SET Status = '$status' WHERE id = '$id'";
    $data = mysqli_query($conn, $query);
    if ($data) {
       echo "<script>alert('User Updated Sucessfully!'); window.location.href='updatemem.php';</script>";
    }
    else {
...
<head>
    ...
    <!-- Title Page-->
    <title>ACTSTI: Academic Adviser Update Members | STICaloocan </title>
...
<body>
     ...
     <div id='preloader' >
      ...
      <div class="page-wrapper" style = "background-color:whitesmoke;background-image:url(images/bg9.jpg);background-size:cover;
...
?>
         ...
         <!-- PAGE CONTAINER-->
         <div style = "background-color:whitesmoke;background-image:url(images/bg9.jpg);background-size:cover;
       ...
       <div class="main-content">
        ...
        color:#000;font-weight:500;cursor:default;">MANAGE CSG MEMBERS</p>
         <div  class="d-flex justify-content-center align-items-center">
                <div style = "background-color:white;width:95%;padding:20px;box-shadow:1px 1px 12px 3px rgba(0,0,0,0.3);
                margin-top:30px;border-radius:10px;" class="table-responsive">  
                    <p style="font-size:35px;margin-bottom:5px;text-align: center;
                    color:#000;font-weight:500;cursor:default;">UPDATE STUDENT / ACADEMIC MEMBERS</p>

                     <table style = "margin-left:2px;color:#000;" id="tabledit" class="table table-striped table-bordered">  
                          <thead>  
...
<?php
$query = "SELECT csgid, usertype,unicode, firstname,lastname,department,Status,RegDate,id,selectbatch,batchyear FROM csgname WHERE unicode != 10";
...
?>  
        ...
        <tr style= "color:black;font-size:15px;font-weight:500;text-align:center;cursor:default;">
            ...
            <td><?php echo $row['csgid'];?></td> 
            <td><?php echo $row['firstname'];?>&nbsp;&nbsp;<?php echo $row['lastname'];?></td>
            <td><?php echo $row['usertype'];?></td> 
            <td><?php echo $row['selectbatch'];?>&nbsp;&nbsp;<br>Batch (<?php echo $row['batchyear'];?>)</td> 
            <td><?php echo $row['department'];?></td>  
            <td><?php $status = $row['Status']; if($status) { ?> 
            <span style="color:green;cursor:default;">ACTIVE</span>
            <?php } else { ?>
            <span style="color:red;cursor:default;">INACTIVE</span>
            <?php }  ?>               
            </td>
            <td style="cursor:default;"><?php echo $row['RegDate'];?></td>  
                <td>

            <a href=editmem.php?csgid=<?php echo $row["id"];?>><i class="fas fa-pencil-alt"></i></a>

            <?php if ($row['Status'] == 1) { ?> 
            <a href="updatemem.php?inid=<?php echo $row ["id"];?>"onclick="return confirm('Are you sure you want to Inactive this <?php echo $row['usertype'];?>?');"> <i style="margin-left:12px;font-size:17px;" class="fas fa-times"></i></a>
            <?php } else { ?> 
                <a href="updatemem.php?id=<?php echo $row ["id"];?>"onclick="return confirm('Are you sure you want to Active this <?php echo $row['usertype'];?>?');"> <i style="margin-left:12px;font-size:17px;"  class="fas fa-check"></i></a>
            <?php } ?>
            </td>  
            </tr>
            <?php $cnt++; } ?>
