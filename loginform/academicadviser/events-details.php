...
include("include/update-eventstatus.php");
...
$query ="UPDATE tblprojects SET IsRead = '$isread' WHERE id = '$did' ";
...
<head>
    ...
    <!-- Title Page-->
    <title>ACTSTI: Academic Adviser Events Details | STICaloocan </title>
...
<body>
    ...
    <div class="page-wrapper"  style = "background-color:whitesmoke;background-image:url(images/bg30.jpg);background-size:cover;
                    ...
                    <ul class="navbar-mobile__list list-unstyled">
                        ...
                        <li class=" has-sub">
                            <a href="approved-project.php">
                                <i class="fas fa-check"></i>Approved Project</a>
                        </li>
                        <li class="has-sub">
                            <a href="declined-project.php">
                                <i class="fas fa-times"></i></i>Declined Project</a>
                        </li>
                        <li>
                            <a href="pending-project.php">
                                 <i class="fas fa-spinner"></i>Pending Project</a>
                        </li>
                        <li class=" has-sub">
                            <a href="addmem.php">
...
<div class="modal fade" id="updateAcademicAdviserModal" tabindex="-1" role="dialog" 
    ...
    <div  class="form-row mb-4">
        <div class="col">
            ...
            class="form-control" autocomplete="off" onchange="loadfile(event)">
        </div>
    </div>  
...
?>
        ...
        <!-- PAGE CONTAINER-->
        <div style = "background-color:whitesmoke;background-image:url(images/bg30.jpg);background-size:cover;
        ...
        <div class="main-content">
         <p style="font-size:30px;margin-top:5px;position:absolute;
          margin-left:20px;font-weight:500;color:#000;cursor:default;">EVENTS DETAILS REQUEST</p>
            <div class="d-flex justify-content-center align-items-center">
             <div id="form-details" style = "background-color:white;margin-top:10px;box-shadow:1px 1px 12px 3px rgba(0,0,0,0.3);border-radius:2%;width:1000px;margin-top:100px;" class=" border border-light p-5">
                    <p style="color:black;margin-bottom:30px!important;font-family:'Montserrat',sans-serif;font-weight:600;cursor:default;border-bottom:3px solid whitesmoke;padding-bottom:10px;font-size:35px;"class="mb-4">EVENTS DETAILS</p>

<?php 
$lid=intval($_GET['eventid']);
$query = "SELECT tblprojects.id as lid, csgname.firstname, csgname.lastname,csgname.csgid, csgname.department, csgname.cpnumber, tblprojects.ProjectType,
tblprojects.budget, tblprojects.file, tblprojects.purpose, tblprojects.manpower,tblprojects.FromDate, tblprojects.ToDate,tblprojects.PostingDate, 
tblprojects.Status, tblprojects.AdminRemark, tblprojects.AdminRemarkDate FROM tblprojects JOIN csgname ON tblprojects.empid = csgname.id 
WHERE tblprojects.id = '$lid'";
$data = mysqli_query($conn, $query);
$result = mysqli_fetch_array($data);
...
?>
    ...
    <div class="form-row mb-4">
        <div class="col">
            <!-- CSG OFFICER -->
             <label style="color:#005baa;font-weight:500;float:left;">CSG NAME:</label>
             <input type="text" class="form-control" style="background-color:#fff;cursor:default;"
             value="<?php echo $row['firstname']?> <?php echo $row['lastname']?>" readonly>
        </div>
        <div class="col">
            <!-- CSG ID -->
             <label style="color:#005baa;font-weight:500;float:left;">CSG ID:</label>
             <input type="text" class="form-control" style="background-color:#fff;cursor:default;"
             value="<?php echo $row['csgid']?>" readonly>
        </div>
    </div>
    <div class = "form-row mb-4">
        <div class="col">
            <!-- CSG CONTACT NO -->
             <label style="color:#005baa;font-weight:500;float:left;">CSG CONTACT NO:</label>
             <input type="text" class="form-control" style="background-color:#fff;cursor:default;"
             value="<?php echo $row['cpnumber']?>" readonly>
        </div>
        <div class="col">
            <!-- DEPARTMENT -->
             <label style="color:#005baa;font-weight:500;float:left;">DEPARTMENT CODE:</label>
             <input type="text" class="form-control" style="background-color:#fff;cursor:default;"
    ...
    <div class = "form-row mb-4">
        <div class = "col">
            <!-- PURPOSE / OBJECTIVE-->
             <label style="color:#005baa;font-weight:500;float:left;">PURPOSE / OBJECTIVE:</label>
             <input type="text" class="form-control" style="background-color:#fff;cursor:default;"
             value="<?php echo $row['purpose']?>" readonly>
        </div>
    </div>
    <div class = "form-row mb-4">
        <div class = "col">
        ...
        <div class = "col">
            <!-- MAN POWER INVOLVED -->
             <label style="color:#005baa;font-weight:500;float:left;">MAN POWER INVOLVED:</label>
             <input type="text" class="form-control" style="background-color:#fff;cursor:default;"
             value="<?php echo $row['manpower']?>"readonly>
        </div>
    </div>
    <div class = "form-row mb-4">
        <div class = "col">
            <!-- DATA FILE (PDF,WORD OR ETC.) -->
            <label style="color:#005baa;font-weight:500;float:left;">FILE:</label>
            <input type="submit" class="form-control" style="background-color:#fff;cursor:default;
             text-align:left;color:#005baa;cursor:pointer;"value="<?php echo $row['file']?>" 
             onclick="window.location.href = '../admin/upload/<?php echo $row['file'];?>';"readonly>
        </div>
        <div class = "col">
            <!-- POSTING DATE  -->
             <label style="color:#005baa;font-weight:500;float:left;">POSTING DATE:</label>
             <input type="text" class="form-control" style="background-color:#fff;cursor:default;"
             value="<?php echo $row['PostingDate']?>"readonly>
        </div>
    </div>
    <div class = "form-row mb-4">
        <div class = "col">
        ...
        <div class = "col">
            <!-- MAN POWER INVOLVED -->
             ...
             <input type="text" class="form-control" style="background-color:#fff;cursor:default;"
             value="<?php echo $row['ToDate']?>"readonly>
        </div>
    </div>
    <div class = "form-row mb-4">
        <div class = "col">
            <!-- PROJECT STATUS-->
             <label style="color:#005baa;font-weight:500;float:left;">PROJECT STATUS:</label>
             <li style="display: inline-block;font-size:17px;color:black;"><?php $status = $row['Status']; 
             if($status == 1) { ?> 
                <span style="color:green;margin-left:5px;">APPROVED</span>
             <?php } if($status == 2 ) { ?>
                <span style="color:red;margin-left:5px;">DECLINED</span>
             <?php } if ($status == 0) {?>
                <span style="color:blue;" class="ml-2">WAITING FOR APPROVAL</p></span>
             <?php } ?></li>
        </div>
        <div class = "col">
            <!-- ADVISER REMARK -->
             <label style="color:#005baa;font-weight:500;float:left;">ADVISER REMARK</label>
             <li style="display: inline-block;font-size:17px;color:black;">
            <?php if ($row['AdminRemark']=="") {
                echo ":&nbsp; NO REMARK";
             } else {
                echo strtoupper(': '.$row['AdminRemark']);
             }
             ?></li>
        </div>
    </div>
    <div class = "form-row mb-4">
        <div class = "col">
            <!-- ADVISER TAKEN DATE -->
             <label style="color:#005baa;font-weight:500;float:left;">ADVISER TAKEN DATE</label>
             <li style="display: inline-block;font-size:17px;color:black;">
            <?php if ($row['AdminRemarkDate']=="") {
                echo ":&nbsp; NO ACTION TAKEN";
            } else {
                echo strtoupper(': '.$row['AdminRemarkDate']);
            }
            ?></li>
        </div>
    </div>
...
{
?>
<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#actionModal">TAKE ACTION</button>
        ...
        <div class="modal fade" id="actionModal" tabindex="-1" role="dialog" 
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content" style="margin-top:150px;">
                    <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel" style="cursor:default;color:#000;font-size:20px;">EVENTS TAKE ACTION</h5>
                    </div>
                    <div class="modal-body">
                <form id="FormEventSubmit" action="" method="post">  
            
                <select style="cursor:pointer;" name="status" id ="selectoption" class="form-control" onChange="NVM();">
                ...
                <div class="form-group mt-3">
                        <label  for="comment" style="margin-bottom:15px;color:#005baa;">ADVISER REMARK:</label>
                        <textarea style="margin-top:-10px;" class="form-control" rows="6" name="description" id="Description" required=""></textarea>
                </div>
                <div  style = "display:none;" class="form-group" id="confirmo">
                        <label  for="comment" style="color:#005baa;font-weight:500;">CONFIRM BUDGET:</label>
                        <input style="color:black;height:44px;margin-top:-10px;" type="text" id="confirmbudget" name="confirmbudget" class="form-control money"  autocomplete="off">
                </div>
                    <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal" id="btnReset">CANCEL</button>
                    <button type="submit"  name="updateEventForm" class="btn btn-primary" onclick="return confirm('Are you sure you want to save changes?')">SAVE CHANGES</button>
                    </div>
                    </form>
                </div>
                </div>
...
$("#btnReset").on("click", function() {
    $("#FormEventSubmit").validate().resetForm();
    document.getElementById('FormSubmit').reset();
});

$('#FormEventSubmit').keyup(function(event) {
