...
<head>
    ...
    <!-- Title Page-->
    <title>ACTSTI: Academic Adviser Student Liquidation Reports | STICaloocan </title>
            ...
            <nav class="navbar-mobile">
                <div class="container-fluid">
                    <ul class="navbar-mobile__list list-unstyled">
                        ...
                        <li>
                            <a href="csgliquidation.php">
                                <i class="fas fa-clipboard-list"></i>BSIT Reports</a>
                        </li>
...
?>
        ...
        <!-- PAGE CONTAINER-->
        <div style = "background-color:whitesmoke;background-image:url(images/bg10.jpg);background-size:cover;
       ...
       <div class="main-content">
        <div class="section__content section__content--p30">
        <p style="font-size:30px;color:#000;cursor:default;font-weight:500;">STUDENT LIQUIDATION REPORTS</p>
            <div class="d-flex justify-content-center align-items-center">
                <div style = "background-color:white;max-width:100%;padding:20px;box-shadow:1px 1px 12px 3px rgba(0,0,0,0.2);
                margin-top:30px;border-radius:10px;" class="table-responsive">
                     <p style="text-align:center;font-size:35px;color:#000;
                     font-weight:500;cursor:default;"> DETAILS OF EXPENSE REPORT </p>
...
$query = "SELECT csgliquidation.id as csid, csgname.firstname, csgname.lastname, (confirmbudget - totalcost) as excessbudget, (totalcost - confirmbudget) as shortagebudget,
csgliquidation.eventdate, csgliquidation.eventtitle,csgliquidation.file, csgliquidation.totalcost,csgliquidation.confirmbudget FROM csgliquidation	JOIN csgname 
ON csgliquidation.empid = csgname.id ORDER BY csid";

$result = mysqli_query($conn, $query);
$cnt = 1;
     while($row = mysqli_fetch_array($result))
 {
    $excessBudget = $row['excessbudget'];
    $shortBudget = $row['shortagebudget'];
?>  
                                            <tr style= "color:black; font-size:16px;font-weight:500;text-align:center;">
                                            <td><?php echo htmlentities($cnt)?></td>
                                            <td><?php echo $row['csid'];?></td>   
                                            <td><?php echo $row['eventdate'];?></td>
                                            <td><?php echo $row['firstname'];?> <?php echo $row['lastname'];?></td>
                                            <td><?php echo $row['eventtitle'];?></td>
                                            <td><a href="../academicadviser/academicstudentliquidation/<?php echo $row['file'];?>"><?php echo $row['file'];?></a></td> 
                                            <td>₱ <?php echo number_format($row['totalcost']);?></td>
                                            <td>₱ <?php echo  number_format($row['confirmbudget']);?></td>
                                            <td id = "exBudget"><?php $excessbudget = $row['excessbudget']; 
                                             if($excessbudget > 0) { ?> 
                                             <span style="color:green;font-weight:500;">₱ <?php echo number_format($excessBudget);?></span>
                                            <?php } else { ?>
                                                <span>₱ 0</span>
                                           <?php }?></td> 
                                            ...
                                            if($shortagebudget > 0) { ?> 
                                             <span id = "shortBudget" style="color:red;font-weight:500;">₱ <?php echo number_format($shortBudget);?></span>
                                            <?php } else { ?>
                                                <span>₱ 0 </span>
                                           <?php }?></td> 
                                        </tr>
